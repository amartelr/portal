...
func main() {
	...
	connStr := "postgres://gouser:!password@localhost/postgres?sslmode=disable"
	db, err := sql.Open("postgres", connStr)
	//db, err := sql.Open("postgres", "user=gouser dbname=postgres sslmode=verify-full")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	rows, err := db.Query("select * from animal where age > $1", 5) //$ instead of ?
	handlerows(rows, err)
...
func testTransaction(db *sql.DB) {
	...
	handlerows(rows, err)
	results, err := tx.Exec("Update animal set age = $1 where id = $2", 18, 2)
	if err != nil {
		log.Fatal(err)
	}
